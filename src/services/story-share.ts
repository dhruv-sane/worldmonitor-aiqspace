...
export function generateStoryDeepLink(
  ...
  return `${location.origin}/api/story?${params.toString()}`;
}
...
export const shareTexts = {
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via GeoSentinel - AI-powered geopolitical intelligence`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - GeoSentinel`)}`,
